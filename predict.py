...
#extrapolation to predict future temp
def extrapol(self):
    a1, n1 = self.window_avg(1, datetime.datetime.now())
    a2, n2 = self.window_avg(2, datetime.datetime.now())
    #Temp prediction=1st temp +( diff of time interval )(diff of temp )/(Time interval)    
    ts = datetime.datetime.now().strftime("%H")
    t=int(ts)
    a3 = a1+(((t+1)-(t-2))*(a2-a1))/((t-1)-(t-2))
    print("The next Hour prediction : ",round(a3,2) ,"°C" )
